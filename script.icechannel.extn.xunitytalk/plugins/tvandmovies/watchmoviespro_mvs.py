...
class watchmoviespro(MovieSource):
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):
        ...
        headers={'Host':'www.watchmovies-pro.com','Origin':'http://www.watchmovies-pro.com','Referer':'http://www.watchmovies-pro.com/search_movies','Connection':'keep-alive','Content-Type':'application/x-www-form-urlencoded'}

        title = self.CleanTextForSearch(title) 
        name = self.CleanTextForSearch(name)

        form_data={ "q":"%s" % (name),"sb":'' }
        content = net.http_POST('http://www.watchmovies-pro.com/search_movies',form_data, headers).content
        
        match=re.compile('<a href="(.+?)">(.+?) \((.+?)\)</a>').findall(content)
        for url_ , _name , _year  in match:
            url='http://www.watchmovies-pro.com'+url_
            if name in _name:
                if year in _year:
